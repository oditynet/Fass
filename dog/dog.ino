...
void step4legstoside(bool left, float STEP_DURATION, int Rcentr = 200);
...
int inverseLinearTransform(int value, int inMin = 0, int inMax = 400, int outMin = 200, int outMax = 1000) {
    // Ограничиваем входное значение в диапазоне [inMin, inMax]
    value = constrain(value, inMin, inMax);
    
    // Вычисляем преобразование с обратной линейностью
...
void processRemoteData(RemoteData data) {
  ...
  int value;
  if (data.leftY < 400) {
    Serial.print("Y_up: "); Serial.println(data.leftY);

    value = inverseLinearTransform(data.leftY);
    gotohome=1;
    stepalllegs(false,value, 0 ); 
  } else if (data.leftY > 600) {
    Serial.print("Y_down: "); Serial.println(data.leftY);

    value = inverseLinearTransform(data.leftY, 600, 1024);
    gotohome=1;
    stepalllegs(true,value,0);
  } else if (data.leftX < 400) {
    Serial.print("X_up: "); Serial.println(data.leftX);

    value = inverseLinearTransform(data.leftX,0,400,1,40);
    gotohome=1;
    heightchange4leg(value); //LEft
  } else if (data.leftX > 600) {
    Serial.print("X_donwn: "); Serial.println(data.leftX);

    value = inverseLinearTransform(data.leftX,600, 1024, -1, -40);
    gotohome=1;
    heightchange4leg(value);//right
  ...
  if (data.rightX < 400) {
    Serial.print("R X_left: "); Serial.println(data.rightX);

    value = inverseLinearTransform(data.rightX);
    gotohome=1;
    step4legstoside(true, value); // Влево
  } else if (data.rightX > 600) {
    Serial.print("R X_right: "); Serial.println(data.rightX);
  
    value = inverseLinearTransform(data.rightX,600, 1024);
    gotohome=1;
    step4legstoside(false,  value); // Вправо
  }
}
